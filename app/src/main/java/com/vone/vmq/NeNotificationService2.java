...
public class NeNotificationService2 extends NotificationListenerService {
            ...
            public void run() {
                ...
                while (isRunning && newThread == Thread.currentThread()) {
                    ...
                    if (sanitizedHost.startsWith("http://") || sanitizedHost.startsWith("https://")) {
                        _url = sanitizedHost + "/appHeart?t=" + t + "&sign=" + sign;
                    } else {
                        _url = "http://" + sanitizedHost + "/appHeart?t=" + t + "&sign=" + sign;
                    }
                    ...
                    call.enqueue(new Callback() {
                        ...
                        public void onFailure(Call call, IOException e) {
                            ...
                            // Toast.makeText(getApplicationContext(), "心跳状态错误，请检查配置是否正确!" + error,
            ...
            if (extras != null) {
                ...
                if ("com.eg.android.AlipayGphone".equals(pkg)) {
                    if (!content.equals("")) {
                        if (content.contains("通过扫码向你付款") || content.contains("成功收款")
                            ...
                            if (money != null) {
                                ...
                                handler.post(new Runnable() {
                                    public void run() {
                                        Toast.makeText(getApplicationContext(), "匹配成功：支付宝到账" + finalMoney + "元",
                                                Toast.LENGTH_LONG).show();
                                    }
                                });
                                ...
                                handler.post(new Runnable() {
                                    public void run() {
                                        Toast.makeText(getApplicationContext(), "监听到支付宝消息但未匹配到金额！", Toast.LENGTH_SHORT)
                                                .show();
                                    }
                                });
                            }
                    ...
                    if (!content.equals("")) {
                        ...
                        if (title.equals("微信") || title.equals("微信支付") || title.equals("微信收款助手")
                                || title.equals("微信收款商业版")
                            ...
                            if (money != null) {
                                ...
                                handler.post(new Runnable() {
                                    public void run() {
                                        Toast.makeText(getApplicationContext(), "匹配成功：微信到账" + finalMoney + "元",
                                                Toast.LENGTH_LONG).show();
                                    }
                                });
                                ...
                                handler.post(new Runnable() {
                                    public void run() {
                                        Toast.makeText(getApplicationContext(), "监听到微信消息但未匹配到金额！", Toast.LENGTH_SHORT)
                                                .show();
                                    }
                                });
                            }
                ...
                } else if ("com.shinian.pay".equals(pkg)) {
                    if (content.equals("这是一条测试推送信息，如果程序正常，则会提示监听权限正常")) {
                        handler.post(new Runnable() {
                            public void run() {
                                Toast.makeText(getApplicationContext(), "监听正常，如无法正常回调请联系作者反馈！", Toast.LENGTH_SHORT)
                                        .show();
                            }
                        });
                    }
    ...
    public void appPush(int type, double price) {
        ...
        // 核心修复：金额格式必须与 URL 参数中的 price 字符串完全一致
        // 将 double 转为去掉多余 0 的字符串，例如 0.10 转为 0.1
        String priceStr = BigDecimal.valueOf(price).stripTrailingZeros().toPlainString();

        // 匹配服务端签名逻辑: md5($type . $price . $t . $transMemo . $key)
        // 这里的 transMemo 留空以适配通用监控
        String transMemo = "";
        ...
        String _url;
        if (sanitizedHost.startsWith("http://") || sanitizedHost.startsWith("https://")) {
            _url = sanitizedHost + "/appPush?t=" + t + "&type=" + type + "&price=" + priceStr + "&sign=" + sign;
        } else {
            _url = "http://" + sanitizedHost + "/appPush?t=" + t + "&type=" + type + "&price=" + priceStr + "&sign="
                    + sign;
        }
        final String url = _url;
